...
def roman_to_arabic(roman):
    ...
    if not _ROMAN_NUMERAL_PATTERN.match(roman):
        raise InvalidRomanNumeralError("Invalid Roman numeral: {}".format(roman))

    arabic = 0
    for i, n in enumerate(roman):
        if (
            i == len(roman) - 1
            or _ROMAN_NUMERAL_MAP[roman[i]] >= _ROMAN_NUMERAL_MAP[roman[i + 1]]
        ):
...
def call(cmd, encoding=None):
    ...
    try:
        ...
        raise CommandError("Command not installed: {}".format(cmd[0])) from e
...
def import_pdftk(data, collapse_level=0):
    ...
    for line in data.splitlines():
        ...
        for bm_type, bm_detail in _BOOKMARK_DESCRIPTION.items():
            ...
            bookmark_info[bm_type]["collapse"] = (
                collapse_level != 0
                and bookmark_info[bm_type]["level"] >= collapse_level
            )
...
def _parse_bookmark_command(line):
    ...
    try:
        ...
        raise InvalidBookmarkSyntaxError("Invalid syntax: {}".format(line)) from e
...
def _parse_level(line, level_indent):
    ...
    if space_count % level_indent != 0:
        raise InvalidBookmarkSyntaxError("Level indentation error: {}".format(line))
...
def _split_title_page(title_page):
    ...
    if start_pos < 0:
        raise InvalidBookmarkSyntaxError(
            'There must be at least {} "." specified in the line "{}"'.format(
                _CONTENT_MINIMUM_DOTS, title_page
            )
        )
...
def import_bmk(bookmark_data, collapse_level=0):
    ...
    for line in bookmark_data.splitlines():
        ...
        if not page_label_saved:
            bookmarks["page_label"].append(
                {
                    kk: vv
                    for kk, vv in page_config.items()
                    if kk in ["new_index", "num_start", "num_style"]
                }
            )
        ...
        collapse = (
            page_config["collapse_level"] != 0
            and level >= page_config["collapse_level"]
        )
...
def _pdfmark_unicode_decode(string):
    ...
    b = bytes(
        int(float.fromhex(x1 + x2)) for x1, x2 in zip(string[5:-2:2], string[6:-1:2])
    )
...
def _write_pdfmark_pagemode():
    ...
    os.write(fd, b"[/PageMode /UseOutlines /View [/FitH] /Page 1 /DOCVIEW pdfmark\n")
    os.close(fd)
    return filename
...
def main():
    ...
    parser.add_argument("-b", "--bookmark", help="the bookmark file to be imported")
    parser.add_argument("-p", "--pdf", help="the input PDF file")
    parser.add_argument("-o", "--output-pdf", help="the output PDF file")
    parser.add_argument(
    ...
    if (
        args.bookmark is None
        and args.pdf is None
        or args.pdf is None
        and args.output_pdf is not None
    ):
    ...
    if args.format == "pdfmark" or (
        args.output_pdf is not None and args.pdf is not None
    ):
